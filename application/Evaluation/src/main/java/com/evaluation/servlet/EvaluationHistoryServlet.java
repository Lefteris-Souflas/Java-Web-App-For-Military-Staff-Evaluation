...
@WebServlet(urlPatterns={"/evaluationHistory"})
public class EvaluationHistoryServlet extends HttpServlet{
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(userInSession==null){//αν δεν έχει συνδεθεί ο χρήστης τον στέλνουμε να κάνει login
            ...
            System.out.println("user in session is: "+userInSession.getIdu());
            try {
                
                list=DBUtils.queryEvaluationHistory(conn, userInSession.getIdu());//φέρνει τα αποτελέσματα
            } catch (SQLException ex) {
               errorString=ex.getMessage();
               hasError=true;
            }//try-catch
            req.setAttribute("evaluationHistory",list);            
            req.setAttribute("errorString", errorString);
            req.setAttribute("user",userInSession);
            RequestDispatcher dispatcher=this.getServletContext().getRequestDispatcher("/WEB-INF/views/evaluationHistory.jsp");
